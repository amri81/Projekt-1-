...
function exporterPDFMois(annee, mois) {
    try {
        ...
        doc.setFontSize(12);
        const sousTitre = selectedLogement === "tous" 
            ? `${moisNoms[mois]} ${annee} - Alle Unterkünfte`
...
function loginUser(e){
  ...
  if (users[user] && users[user].password === pass) {
    ...
    window.location.href = "index.html";   // Hauptseite
  } else {
    document.getElementById("error").innerText = "❌ Benutzer oder Passwort falsch.";
  }
  return false;
}
